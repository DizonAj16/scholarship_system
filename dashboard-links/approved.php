...
try {
    ...
    $countStmt = $pdo->prepare("SELECT COUNT(*) FROM scholarship_applications WHERE status = 'Approved'");
    $countStmt->execute();
    $totalRecords = $countStmt->fetchColumn();

    // Fetch records for the current page
    $stmt = $pdo->prepare("SELECT * FROM scholarship_applications WHERE status = 'Approved' ORDER BY application_id LIMIT :limit OFFSET :offset");
...
<body>
    ...
    <div class="print-header">
        <h1>APPROVED SCHOLARSHIP APPLICATIONS</h1>
    ...
    <main>
        <h2 class="no-print">List of Approved Applications</h2>
        ...
        <div class="print-summary">
            ...
            Total Approved Applications: <?php echo $totalRecords; ?><br>
            <?php if ($limit === 'all'): ?>
                Displaying: All <?php echo $totalRecords; ?> records
            <?php else: ?>
                Displaying: <?php echo count($approvedApplications); ?> records (Page <?php echo $page; ?> of <?php echo $totalPages; ?>)
            <?php endif; ?>
        </div>
        ...
        <?php if (count($approvedApplications) > 0): ?>
            ...
            <?php endif; ?>
        <?php else: ?>
            <p class="no-data">No approved applications found.</p>
        <?php endif; ?>
